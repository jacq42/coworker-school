...
class TaskGeneratorFlowTests(unittest.TestCase):
    ...
    def test_parse_lessons_for_subject_preserves_extensions(self) -> None:
        self.assertEqual(_parse_lessons_for_subject("TMS2", "Englisch"), ["TMS2"])
        self.assertEqual(_parse_lessons_for_subject("unitTMS2", "Englisch"), ["TMS2"])
    ...
    def test_available_lessons_are_filtered_by_language(self) -> None:
        ...
        self.assertIn("TMS1", english_lessons)
        self.assertEqual(_available_lessons_for_subject("Math"), [])

    def test_english_lesson_labels_include_extensions(self) -> None:
        english_labels = _available_lesson_labels_for_subject("Englisch")
        self.assertIn("1", english_labels)
        self.assertIn("AC1", english_labels)
        self.assertIn("TMS1", english_labels)
        self.assertIn("Trailer1", english_labels)
    ...
    def test_save_task_writes_worksheet_and_solution_files_for_structured_output(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            with patch.object(TaskGeneratorFlow, "_project_root", return_value=Path(tmp_dir)):
                flow.save_task()

            worksheet_matches = list(Path(tmp_dir).glob("aufgaben/latein/worksheet_*.md"))
            solution_matches = list(Path(tmp_dir).glob("aufgaben/latein/solution_*.md"))
            manifest_path = os.path.join(tmp_dir, "output_manifest.json")
            self.assertEqual(len(worksheet_matches), 1)
            self.assertEqual(len(solution_matches), 1)
            self.assertTrue(os.path.exists(manifest_path))

            worksheet_path = str(worksheet_matches[0])
            solution_path = str(solution_matches[0])
            ...
            with open(solution_path, "r", encoding="utf-8") as solution_file:
                self.assertEqual(solution_file.read(), "# Lösung\n")
            ...
            self.assertEqual(
                ...
                [
                    str(Path(worksheet_path).resolve()),
    ...
    def test_save_task_falls_back_to_task_txt_for_plain_output(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            task_matches = list(Path(tmp_dir).glob("task_*.txt"))
            manifest_path = os.path.join(tmp_dir, "output_manifest.json")
            self.assertEqual(len(task_matches), 1)
            self.assertTrue(os.path.exists(manifest_path))
            task_path = str(task_matches[0])
            with open(task_path, "r", encoding="utf-8") as task_file:
                self.assertEqual(task_file.read(), "legacy output")
            with open(manifest_path, "r", encoding="utf-8") as manifest_file:
                manifest_payload = json.load(manifest_file)
            self.assertEqual(manifest_payload["files"], [str(Path(task_path).resolve())])
